<div>
    <div class="row TV-writersBlockArea customHeder">
        <div class="RightTvSection ">
            <div class="row BlackHeaderBlock">
                <div class="row">
                    <a href="videos/AllVideos">
                        <p class="TitleBlocksHeader">
                            +TV
                            <img class="WhiteTvs" src="images/them/Iconlive-tv-White.png" alt="Iconlive" />
                        </p>
                        <img src="images/them/redcorner.png">
                    </a>
                </div>
            </div>
            <div class=" TvsBlockHome">
                <div class="row BackGroundGradient">
                    <div class="fullscreenTvs">
                        <div class="TvSection">
                            <div class="LeaguesNews">
                                <a href="section/49/أخبار-عالمية/1">
                                    <p>
                                        أخبار عالمية
                                    </p>
                                </a>
                            </div>
                            <a href="Article/26528/روبوت-التحكيم-جاهز-للظهور-في-مونديال-2022">
                                <img src="https://koraplus.com/images/them/2022/6/large/164336757069520220609030434434.jpg"
                                    class="OneTvSectionIcon" onerror="this.src='images/them/onerror/Large789x539.png';"
                                    title="روبوت للتحكيم" alt="روبوت للتحكيم">
                                <img src="images/them/tvicon.png" class="IconPlayTVs" alt="tvicon">
                                <div class="overlayImg"></div>
                            </a>
                            <div class="OneNewsTitleON">
                                <h3>
                                    <a class="OneNewsTitleOne"
                                        href="Article/26528/روبوت-التحكيم-جاهز-للظهور-في-مونديال-2022">روبوت
                                        التحكيم جاهز للظهور في مونديال 2022</a>
                                </h3>
                                <div class="row TVsDate">
                                    <i class="fa fa-clock-o" aria-hidden="true"></i>
                                    <p>12ساعة</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="TabsVideoTVs">
                        <ul class="tabsDMHeaderTVs tabs">
                            <li class="tab-link current" data-tab="tabTVs-1"> الأكثر مشاهدة</li>
                        </ul>
                        <div id="tabTVs-1" class="tab-content  current">
                            <div class="row TVsThreeCard  TVsThreeCardHome">
                                <div class="card">
                                    <div class="LeaguesNews">
                                        <a href="section/7/الدورى-المصرى/1">
                                            <p>
                                                الدورى المصرى
                                            </p>
                                        </a>
                                    </div>
                                    <a class="card-img-link"
                                        href="Article/26499/حوار-بلس-كريم-رمزي-يكشف-كواليس-خاصة-بمستقبل-موسيماني-مع">
                                        <img src="https://koraplus.com/images/them/2022/6/large-3/162671102047720220608040826826.jpg"
                                            class="card-img-top" onerror="this.src='images/them/onerror/Large264x180.png';"
                                            title="موسيماني" alt="موسيماني">
                                        <img src="images/them/tvicon.png" class="IconPlayDontMiss" alt="tvicon">
                                    </a>
                                    <div class="card-body">
                                        <a href="Article/26499/حوار-بلس-كريم-رمزي-يكشف-كواليس-خاصة-بمستقبل-موسيماني-مع">
                                            <p class="card-text">حوار بلس | كريم رمزي يكشف كواليس خاصة بمستقبل
                                                موسيماني مع الأهلي ما بين الرحيل والبقاء</p>
                                        </a>
                                    </div>
                                </div>
                                <div class="card">
                                    <div class="LeaguesNews">
                                        <a href="section/31/كأس-أمم-إفريقيا/1">
                                            <p>
                                                كأس أمم إفريقيا
                                            </p>
                                        </a>
                                    </div>
                                    <a class="card-img-link"
                                        href="Article/26441/حوار-بلس-سوء-حظ-للمنتخب-المصري-جائزة-جديدة-لمحمد-صلاح">
                                        <img src="https://koraplus.com/images/them/2022/6/large-3/2871874097202206051058465846.jpg"
                                            class="card-img-top" onerror="this.src='images/them/onerror/Large264x180.png';"
                                            title="منتخب مصر" alt="منتخب مصر">
                                        <img src="images/them/tvicon.png" class="IconPlayDontMiss" alt="tvicon">
                                    </a>
                                    <div class="card-body">
                                        <a href="Article/26441/حوار-بلس-سوء-حظ-للمنتخب-المصري-جائزة-جديدة-لمحمد-صلاح">
                                            <p class="card-text">حوار بلس | سوء حظ للمنتخب المصري.. جائزة جديدة
                                                لمحمد صلاح.. وحسم مستقبل فرجاني</p>
                                        </a>
                                    </div>
                                </div>
                                <div class="card">
                                    <div class="LeaguesNews">
                                        <a href="section/20/اسكواش/1">
                                            <p>
                                                اسكواش
                                            </p>
                                        </a>
                                    </div>
                                    <a class="card-img-link"
                                        href="Article/26304/حكاية-محمد-الشوربجي-من-الالتفاف-بعلم-مصر-إلى-اللعب-باسم">
                                        <img src="https://koraplus.com/images/them/2022/6/large-3/2871874097202206010150375037.jpg"
                                            class="card-img-top" onerror="this.src='images/them/onerror/Large264x180.png';"
                                            title="الشوربجي" alt="الشوربجي">
                                        <img src="images/them/tvicon.png" class="IconPlayDontMiss" alt="tvicon">
                                    </a>
                                    <div class="card-body">
                                        <a href="Article/26304/حكاية-محمد-الشوربجي-من-الالتفاف-بعلم-مصر-إلى-اللعب-باسم">
                                            <p class="card-text">حكاية محمد الشوربجي من الالتفاف بعلم مصر إلى
                                                اللعب
                                                باسم إنجلترا</p>
                                        </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="LeftTvSection ">
            <div class="writersBlockArea">
                <div class="row BlackHeaderBlock">
                    <a href="Editor/Index" class="row">
                        <p class="TitleBlocksHeader">
                            مقالات
                        </p>
                    </a>
                    <img src="images/them/Iconwriters.png" style="height:36px" alt="Iconwriters" />
                </div>
                <div class="swiperwriters">
                    <div class="swiper-wrapper">
                        <div class="swiper-slide">
                            <div class="row writersBlockCenter">
                                <a href="Article/26480/إيطاليا-مع-مانشيني-الفوقان-بعد-فوات-الأوان">
                                    <img src="https://koraplus.com/images/them/2022/6/large/1619696029370202206081059505950.jpg"
                                        onerror="this.src='images/them/onerror/Large789x539.png';" title="إبراهيم جاد"
                                        alt="إبراهيم جاد">
                                </a>
                                <div class="writerDetailsHead">
                                    <a href="Article/26480/إيطاليا-مع-مانشيني-الفوقان-بعد-فوات-الأوان"
                                        class="writerArticleTitle">
                                        إيطاليا مع مانشيني.. الفوقان بعد فوات الأوان
                                    </a>
                                    <div class="row writerName">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="row writersBlockCenter">
                                <a href="Article/25273/خدعوك-فقالوا-بنزيما-هو-الأفضل">
                                    <img src="https://koraplus.com/images/them/2022/1/large/287187409720220110035000500.jpg"
                                        onerror="this.src='images/them/onerror/Large789x539.png';" title="محمد عمارة"
                                        alt="محمد عمارة">
                                </a>
                                <div class="writerDetailsHead">
                                    <a href="Article/25273/خدعوك-فقالوا-بنزيما-هو-الأفضل" class="writerArticleTitle">
                                        &quot;خدعوك فقالوا.. بنزيما هو الأفضل&quot; !
                                    </a>
                                    <div class="row writerName">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="row writersBlockCenter">
                                <a href="Article/25161/عفاريت-كورتوا-ونجاح-صلاح">
                                    <img src="https://koraplus.com/images/them/2022/5/large/2871874097202205290421252125.jpg"
                                        onerror="this.src='images/them/onerror/Large789x539.png';" title="عمرو الجرزاوي"
                                        alt="عمرو الجرزاوي">
                                </a>
                                <div class="writerDetailsHead">
                                    <a href="Article/25161/عفاريت-كورتوا-ونجاح-صلاح" class="writerArticleTitle">
                                        عفاريت &quot;كورتوا&quot; .. ونجاح &quot;صلاح&quot;
                                    </a>
                                    <div class="row writerName">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="row writersBlockCenter">
                                <a href="Article/24680/الملهمة-بنت-الجيران-وحب-الطفولة-التي-تزوجها-محمد-صلاح">
                                    <img src="https://koraplus.com/images/them/2022/6/large/287187409720220601074102412.jpg"
                                        onerror="this.src='images/them/onerror/Large789x539.png';" title="محمد غنيم"
                                        alt="محمد غنيم">
                                </a>
                                <div class="writerDetailsHead">
                                    <a href="Article/24680/الملهمة-بنت-الجيران-وحب-الطفولة-التي-تزوجها-محمد-صلاح"
                                        class="writerArticleTitle">
                                        الملهمة.. بنت الجيران وحب الطفولة التي تزوجها محمد صلاح
                                    </a>
                                    <div class="row writerName">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="row writersBlockCenter">
                                <a href="Article/24470/التسديد-حلو-وبسمسم-رسالة-إلى-رمضان-صبحي">
                                    <img src="https://koraplus.com/images/them/2022/5/large/2871874097202205240440494049.jpg"
                                        onerror="this.src='images/them/onerror/Large789x539.png';" title="محمد داود"
                                        alt="محمد داود">
                                </a>
                                <div class="writerDetailsHead">
                                    <a href="Article/24470/التسديد-حلو-وبسمسم-رسالة-إلى-رمضان-صبحي"
                                        class="writerArticleTitle">
                                        التسديد حلو وبسمسم.. رسالة إلى رمضان صبحي
                                    </a>
                                    <div class="row writerName">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="row writersBlockCenter">
                                <a href="Article/23840/موسيماني-الإفريقي-وموسيمانى-المحلي">
                                    <img src="https://koraplus.com/images/them/2022/4/large/287187409720220410110148148.jpg"
                                        onerror="this.src='images/them/onerror/Large789x539.png';" title="مروان الشافعي"
                                        alt="مروان الشافعي">
                                </a>
                                <div class="writerDetailsHead">
                                    <a href="Article/23840/موسيماني-الإفريقي-وموسيمانى-المحلي"
                                        class="writerArticleTitle">
                                        موسيماني الإفريقي وموسيمانى المحلي
                                    </a>
                                    <div class="row writerName">
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="swiper-pagination"></div>
            </div>
            <div class="mostWatched">
                <div class="HeadWatched">
                    <h3>الأكثر قراءة</h3>
                </div>
                <ul class="listWatched">
                    <li>
                        <span>1</span>
                        <a href="Article/26507/محمود-شبانة-يحسم-موقف-من-العودة-إلى-الزمالك-بعد-نهاية"> محمود شبانة
                            يحسم موقف من العودة إلى الزمالك بعد نهاية إعارته للاتحاد السكندري </a>
                    </li>
                    <li>
                        <span>2</span>
                        <a href="Article/26496/مواعيد-مباريات-اليوم-الخميس-9-–-6-–-2022-والقنوات"> مواعيد مباريات
                            اليوم الخميس 9 – 6 – 2022 والقنوات الناقلة.. مصر مع إثيوبيا </a>
                    </li>
                    <li>
                        <span>3</span>
                        <a href="Article/26565/نهاية-المباراة-مصر-0-&#215;-2-إثيوبيا-تصفيات-كأس-أمم"> نهاية
                            المباراة.. مصر 0 &#215; 2 إثيوبيا.. تصفيات كأس أمم أفريقيا 2023 </a>
                    </li>
                    <li>
                        <span>4</span>
                        <a href="Article/26517/بطاقة-جديدة-وفرصة-للزمالك-بطل-العرب-في-اليد-يشارك-في"> بطاقة جديدة
                            وفرصة للزمالك.. بطل العرب في اليد يشارك في مونديال الأندية </a>
                    </li>
                    <li>
                        <span>5</span>
                        <a href="Article/26547/مروان-صلاح-الحارس-الذي-تفوق-على-2000-موهبة-ليصل-إلى"> مروان صلاح..
                            الحارس الذي تفوق على 2000 موهبة ليصل إلى بايرن ميونخ &quot;تقرير&quot; </a>
                    </li>
                    <li>
                        <img src="images/them/logo03.png" alt="logo03">
                    </li>
                </ul>
            </div>
        </div>
    </div>
</div>
